    ...
    def action_check_card(self):
        if not self.serial:
            ...
            if len(lot) == 0:
                ...
                # Đôi với tài khoản quản lý sẽ được tra cứu tất cả các khách hàng
                if sale_team_manage:
                    ...
                    for i in x_customer:
                        if i.x_brand_id.id == self.brand_id.id:
                            ...
                            break
                # Đối với tài khoản all sẽ được tra cứu khách hàng thuộc chi nhánh của mình
                if sale_team_all:
                    x_customer = self.env['res.partner'].sudo().search(
                        ['|', ('phone', '=', serial),
                         ('mobile', '=', serial)])
                    for i in x_customer:
                        if i.x_brand_id.id == self.brand_id.id:
                            x_customer_id = x_customer
                            break
                # Đối với quyền only thì chỉ tra cứu được tài khoản khách hàng của mình
                if sale_team_only:
                    x_customer = self.env['res.partner'].sudo().search(
                        ['|', ('phone', '=', serial),
                         ('mobile', '=', serial)])
                ...
                if x_customer_id.id == False:
                    if x_customer:
                        for i in x_customer:
                            if i.x_brand_id.id == self.brand_id.id:
                                raise except_orm("Cảnh báo!", ('Vui lòng liên hệ quản lý để kiểm tra thông tin'))
                            else:
