...
class KsDashboardNinjaBoard(models.Model):
    ...
    def ks_fetch_dashboard_data(self, ks_dashboard_id, ks_item_domain=[]):
        ...
        if len(self.ks_dashboard_items_ids.search(
            ...
            if ks_item_domain:
                try:
                    items = self.ks_fetch_item(self.ks_dashboard_items_ids.search(
                        [['ks_dashboard_ninja_board_id', '=', ks_dashboard_id]] + ks_item_domain).ids, ks_dashboard_id)
                except Exception as e:
                    items = self.ks_fetch_item(self.browse(ks_dashboard_id).ks_dashboard_items_ids.ids, ks_dashboard_id)
                        ...
                        for line in ks_action_lines:
                            if line['ks_item_action_field']:
                                ...
                                if ks_record_id:
                                    ...
                                    line['ks_item_action_field'] = False
    ...
    def ks_prepare_item(self,item):
        ...
        if item.get('ks_list_target_deviation_field'):
