...
class RptRevenueAllocationSum(models.TransientModel):
    _name = 'rpt.revenue.allocation.sum'
    ...
    crm_team_ids = fields.Many2many('crm.team', string="Team", domain=_domain_team_id)
    ...
    def _onchange_crm_team_ids(self):
        ...
        if group_business_manager or group_revenue_accountant or group_cost_accountant or group_revenue_control or group_cashier:
            all_employee_ids = []
            for team in self.crm_team_ids:
                employee_ids = team.get_all_employee_ids()
                if employee_ids:
            ...
            return {
                'domain': {
                    ...
                    'employee_ids': ['|', ('id', 'in', all_employee_ids), ('job_id.x_code', '=', 'BS')]
                }
            }
        if group_consultant or group_therapist:
            employee = EmployeeObj.search(['|', ('user_id', '=', self._uid), ('x_user_ids', 'in', [self._uid])])
